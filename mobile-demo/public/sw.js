...
function getPendingReadings() {
  // Get pending readings from localStorage
  const readings = localStorage.getItem('pending-readings');
  return readings ? JSON.parse(readings) : [];
}

function clearPendingReadings() {
  // Clear synced readings from localStorage
  localStorage.removeItem('pending-readings');
}
